...
interface MaquinaEditarModalProps {
    isOpen: boolean
    onClose: () => void
...
const MaquinaEditarModal: React.FC<MaquinaEditarModalProps> = ({ isOpen, onClose, onSave, maquina }) => {
    ...
    // Carregar dados da máquina quando o modal abrir
    useEffect(() => {
        if (maquina) {
            setFormData({
                ...
                dataFabricacao: maquina.dataFabricacao ? maquina.dataFabricacao.split('T')[0] : '',
                dataInstalacao: maquina.dataInstalacao ? maquina.dataInstalacao.split('T')[0] : '',
                valorCompra: maquina.valorCompra ? maquina.valorCompra.toString() : '',
                eficiencia: maquina.eficiencia ? maquina.eficiencia.toString() : '',
                localizacao: maquina.localizacao || '',
                responsavel: maquina.responsavel || '',
                especificacoes: maquina.especificacoes || '',
                observacoes: maquina.observacoes || '',
                proximaManutencao: maquina.proximaManutencao ? maquina.proximaManutencao.split('T')[0] : '',
                ultimaManutencao: maquina.ultimaManutencao ? maquina.ultimaManutencao.split('T')[0] : '',
                horasTrabalhadas: maquina.horasTrabalhadas ? maquina.horasTrabalhadas.toString() : '',
                horasManutencao: maquina.horasManutencao ? maquina.horasManutencao.toString() : ''
            })
        }
    }, [maquina])
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        if (validateForm() && maquina) {
            ...
            const maquinaAtualizada: Maquina = {
                ...
                dataFabricacao: formData.dataFabricacao || undefined,
                dataInstalacao: formData.dataInstalacao || undefined,
                valorCompra: formData.valorCompra ? Number(formData.valorCompra) : undefined,
                eficiencia: formData.eficiencia ? Number(formData.eficiencia) : undefined,
                localizacao: formData.localizacao || undefined,
                responsavel: formData.responsavel || undefined,
                especificacoes: formData.especificacoes || undefined,
                observacoes: formData.observacoes || undefined,
                proximaManutencao: formData.proximaManutencao || undefined,
                ultimaManutencao: formData.ultimaManutencao || undefined,
                horasTrabalhadas: formData.horasTrabalhadas ? Number(formData.horasTrabalhadas) : undefined,
                horasManutencao: formData.horasManutencao ? Number(formData.horasManutencao) : undefined
            }

            onSave(maquinaAtualizada)
            setErrors({})
            onClose()
        }
    }
...
export default MaquinaEditarModal
